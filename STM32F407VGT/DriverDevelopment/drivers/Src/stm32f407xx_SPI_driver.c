...
void SPI_SendData(SPI_RegDef_t *pSPIx,uint8_t *pTxBuffer, uint32_t Len)
{
	while(Len > 0)
	{
		...
		{
			//16 bit DFF
			//1. load the data in to the SPI_DR
	...
	while(Len > 0)
		{
			...
			{
				//16 bit DFF
				//1. load the data from SPI_DR to Rxbuffer address
...
uint8_t SPI_SendDataIT(SPI_Handle_t *pSPIHandle,uint8_t *pTxBuffer, uint32_t Len)
{
	...
	if(state != SPI_BUSY_IN_TX)
	{
		...
		//3. Enable the TXEIE control bit to get interrupt whenever TXE flag is set in SPI_SR
...
uint8_t SPI_ReceiveDataIT(SPI_Handle_t *pSPIHandle, uint8_t *pRxBuffer, uint32_t Len)
{
	...
	if(state != SPI_BUSY_IN_RX)
	{
		...
		//3. Enable the RXNEIE control bit to get interrupt whenever RXNEIE flag is set in SPI_SR
